...
> 实施状态（2026-08-04）：V040-00～V040-07 已进入 RC；V040-08 的文档、自动化、包体及
> Halo 2.23.3/2.25.4 + Moment 1.15/1.16 本机隔离运行时矩阵、identity Secret 迁移与合成
> 备份恢复已完成；Halo 平台依赖已形成可复现镜像审计与逐项归因，但平台升级/正式风险接受仍未
> 通过；双真机、真实微信、目标环境暗部署/恢复/回滚也仍未通过。另已确认
...
> 2026-08-04 复验 Node 自动化为 193/193、插件双 API 自动化均为 143/143，最低 API clean build
> 产物为 172,162 bytes（SHA-256：`ca4f547d86817ddf0493e5210ef27ac5c3af22219f2e28fd6044fd19012cb34a`）；微信开发者工具
> preview 总包体复验为 253,850 bytes；本轮
> 补充了受限 HTML 实体/控制字符协议清理、资源名与详情路由统一校验/编码、坏路由参数 fail-closed，
> 以及插件关闭/能力探测失败时过滤陈旧 Moment 搜索结果并保留文章主链路、运行时配置显式不可用时 fail-closed、资源协议/API 输入、页面卸载竞态、插件配置边界和错误状态码收口。上述结果仍不替代真实微信、双真机、目标
> 环境暗部署和合法域名门禁。
> 编制日期：2026-08-01
...
## 7. P1：可拆到 v0.4.1 的范围（本地实现已完成，外部门禁待执行）

V040-09～V040-10 的插件/客户端代码、契约文档和自动化测试已在当前工作树完成。该实现仍按
v0.4.1 管理，v0.4.0 生产远程开关保持关闭；真实微信、双真机、内容审核、目标环境暗部署和
正式发布门禁尚未完成。
...
- [x] 三处小程序版本及插件版本一致；
- [x] 新开关默认关闭，旧插件/旧客户端组合安全（自动化/契约范围）；
- [x] 缓存配置不能开启登录或 Moment 写入；
- [x] `PluginMoments` 名称固定，部署者不能通过客户端配置任意探测路径。

### 11.2 瞬间读取与媒体

- [x] Moment 1.15.x 与 1.16.1 的列表/详情可转换；
- [x] 插件未安装、停用、available 超时、返回 HTML/非法 JSON 时首页不出现死入口（自动化范围）；
- [x] 首屏、刷新、分页和标签切换不重复、不串页（自动化/序列保护范围）；
- [x] 相对作者头像和相对媒体 URL 正确补全；
- [ ] PHOTO 1～9 张布局和预览正确；
- [ ] VIDEO 不自动播放，失败有提示，切页后停止；
- [ ] AUDIO 同时只播放一个，页面卸载后无后台残留；
- [x] POST 和未知媒体类型均有安全降级；
- [x] HTML 中 script/iframe/style/事件属性/javascript 链接仍被清理；
- [ ] PRIVATE、未审核、已删除 Moment 不出现在匿名小程序响应中。

### 11.3 搜索、点赞与分享

- [x] 文章、瞬间和混合结果均正确高亮与导航；
- [x] Moment 插件停用后陈旧索引结果不制造死链；
- [x] Post / Moment 点赞 tracker 的 group/plural 正确；
- [x] 两类本地点赞 key 不冲突，旧 Post 状态兼容；
- [x] 重复点击与请求失败不虚增统计（文章/Moment 页面自动化范围）；
- [x] Moment 分享深链在正常、插件关闭和内容删除三种状态下可恢复（自动化范围；真实微信/真机 PENDING）。

### 11.4 登录与资料

- [x] 未主动同意隐私政策时不创建 WeAppUser；
- [x] 同一 OpenID 首次并发登录只创建一个资源；
- [x] 客户端 storage 中没有 token、OpenID 或身份摘要；
- [x] 插件资源、公开 config、错误响应和日志中没有原始 OpenID；
- [x] 登录后昵称可跨冷启动恢复并预填评论（auth-session/评论弹层页面自动化范围）；
- [x] 隐私版本变化后停止静默恢复，重新同意前不能修改资料；
- [x] token 过期、插件重启和 401 只触发一次单飞重登（自动化范围）；
- [x] 退出后当前 token 立即失效，缓存资料和登录意愿清理；
- [x] 注销后二次查询返回未登录/不存在，相关会话全部失效；
- [x] 注销说明明确既有公开评论不会自动删除；
- [x] readerAccount 远程关闭后不再登录，但文章与公开 Moment 仍可阅读。

### 11.5 P1 瞬间评论（本地代码/自动化已完成；v0.4.1 外部门禁待执行）

- [x] 客户端不能传入任意 group/kind/version（服务端固定 Subject，自动化范围）；
- [x] 不存在、私有、未审核或已删除 Moment 不能写评论（Public API/夹具契约范围）；
- [x] Post / Moment 父评论回复均校验到正确主体；
- [x] 其他主体的评论不能借回复 route 绕过；
- [x] Moment 评论同样经过会话、频控、幂等与仅 pass 放行的 msgSecCheck；
- [x] 文章 comment-thread 抽取后原有评论分页、回复和错误分支无回归；
- [x] 远程关闭 Moment 评论后不影响文章评论。
...
- [x] 主包/总包体积满足限制，新增资源未误打包 docs/tests/fixtures；
- [ ] 插件 v0.2.0 先关闭新功能暗部署，小程序 v0.4.0 审核时无新增 UGC 入口；
- [x] 凭据、OpenID、token、identityKey 和测试真实资料扫描无命中（现有扫描证据范围）。
...
V040-09～V040-10 的本地实现不阻塞 v0.4.0，但必须明确标记为 v0.4.1；在真实微信、双真机、
内容审核、目标环境演练和正式审核完成前，不能在远程配置中宣称 Moment 评论已可用。
