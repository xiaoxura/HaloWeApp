...
3. 修改 `config/index.js` 中的 `baseUrl` 为你的博客地址；该文件只保留版本号和站点地址，
   **不要写入 Halo 管理员 PAT、AppSecret 或其他长期凭据**
4. 安装并启用配套 Halo 插件
   [plugin-halo-weapp](https://github.com/xiaoxura/plugin-halo-weapp)，在插件设置中配置
   博客名称、简介、分页、字体及评论/公告等运行时选项
5. 安装依赖并构建 npm：
   ```bash
   npm install
   ```
...
6. 小程序后台 → 开发管理 → 服务器域名：
   ...
   - 图片、附件或插件配置的字体若使用独立 CDN（如对象存储），`downloadFile 合法域名`
     也要一并添加，否则正文资源和字体在真机上无法加载（开发版可勾选“不校验合法域名”跳过）

## 目录结构

```
├── app.js / app.json / app.wxss   # 入口、全局配置、设计规范与页面状态样式
├── config/index.js                # 仅版本号与 Halo 博客地址（不存放凭据）
├── utils/
...
│   ├── plugin-contract.js         # 配套插件名称与 API 路径（双端固定协议）
│   ├── runtime-config.js          # 插件配置：白名单校验、缓存、写能力 fail-closed
...
- **固定协议**：插件名及 `/apis/api.weapp.halo.run/v1alpha1` 路径由双端协议固定，部署者
  无需在小程序中重复填写；插件不可用、超时、HTML 或字段非法时回退安全默认值。
- **配置归属**：博客名称、简介、分页大小、字体、评论、公告、最低版本和隐私政策均在
  Halo 插件 Setting 中维护并由公开白名单 DTO 下发；小程序本地不提供覆盖开关。
- **读写分离**：`commentEnabled` 控制评论区展示；`commentOptions.submitEnabled` /
  `replyEnabled` 单独控制写入口。
...
- **无管理员 Token**：读者端使用 Halo Public API，无需 PAT。管理员 PAT 一旦写入
  `config/index.js` 就会随小程序包公开，因此禁止打包到客户端；未来管理能力必须经服务端
  权限网关实现。
...
- 已实现计划（v0.3.0）：[docs/development-plan-v0.3.0.md](docs/development-plan-v0.3.0.md)
- 下一阶段计划（v0.4.0）：[docs/development-plan-v0.4.0.md](docs/development-plan-v0.4.0.md)
