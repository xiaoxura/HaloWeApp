...
> v0.4.0 生产默认**不开放 Moment 评论/回复**，也不提供私有 Moment、作者发布、Halo 管理员
> 登录或头像上传。Moment 评论的 v0.4.1 P1 契约已实现但仍需独立真机、审核和目标环境门禁，
> 不能把远程关闭的实现解释为生产可用。
...
v0.4.0 RC 当前有 193 项 Node 测试，覆盖文章/评论/搜索/Moment adapter、能力探测、媒体生命周期、
点赞迁移、资源名/路由安全、请求错误、远程配置、读者身份、401 单飞恢复和评论会话复用。测试不等于真机证据；
...
| 当前工作树（资源协议/API 输入/页面竞态与插件配置、错误契约收口） | 253,850 bytes（247.9 KB） |
| 相对 v0.3.0 | +91,317 bytes（+89.2 KiB，+56.18%） |
